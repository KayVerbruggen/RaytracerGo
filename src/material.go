...
func (m material) scatter(rIn ray, hr *hitRecord, atten *vec3, rOut *ray) bool {
	// Difference between diffuse and metallic materials.
	if m.diffuse {
		target := hr.p.add(hr.normal).add(randInUnitSphere())
	...
	if m.fuzz == 0.0 {
		...
		*rOut = ray{hr.p, reflected.add(randInUnitSphere().subScalar(m.fuzz))}
	}
	*atten = m.col
